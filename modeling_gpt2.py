"""PyTorch OpenAI GPT-2 base model."""
...
class GPT2Block(nn.Module):
    ...
    def forward(
        ...
        # residual connection
        hidden_states = self.ln_1(residual + attn_output)
        ...
        # residual connection
        hidden_states = self.ln_2(residual + feed_forward_hidden_states)
...
class GPT2Model(GPT2PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.emb_layer_norm = nn.LayerNorm(self.embed_dim, eps=config.layer_norm_epsilon)

        self.drop = nn.Dropout(config.embd_pdrop)
        self.h = nn.ModuleList([GPT2Block(config) for _ in range(config.num_hidden_layers)])
        self.one_more = nn.Linear(self.embed_dim, self.embed_dim)
    ...
    def parallelize(self, device_map=None):
        ...
        self.emb_layer_norm = self.emb_layer_norm.to(self.first_device)
        # Load onto devices
        ...
        # ln_f to last
        self.one_more = self.one_more.to(self.last_device)
    ...
    def deparallelize(self):
        ...
        self.emb_layer_norm = self.emb_layer_norm.to("cpu")
        for index in range(len(self.h)):
            self.h[index] = self.h[index].to("cpu")
        self.one_more = self.one_more.to("cpu")
    ...
    def forward(
        ...
        hidden_states = self.ln_f(gelu(self.one_more(hidden_states)))
